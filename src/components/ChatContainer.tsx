    ...
    >
      ...
      >
        {messages.map((msg) => (
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between w-full max-w-[85%] mb-1 px-1 text-[10px] font-mono tracking-widest text-slate-500">
              ...
              {msg.sender === 'assistant' && onSpeakMessage && (
                <button
                  onClick={() => {
                  ...
                  className="hover:text-cyan-400 transition-colors p-0.5 rounded cursor-pointer"
                >
                  {isSpeaking ? (
      ...
      {isListening && (
        <div className="px-4 py-2 bg-cyan-950/40 border-t border-cyan-500/30 flex items-center justify-between font-mono text-xs text-cyan-300 animate-pulse shrink-0">
          <div className="flex items-center gap-2 truncate">
            <Mic className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
            <span className="font-bold shrink-0">LISTENING:</span>
            <span className="text-slate-200 italic truncate">{transcript || 'Speak now...'}</span>
          </div>
          <span className="text-[10px] text-cyan-400 font-bold bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/30 shrink-0">
            REAL-TIME STT
          </span>
        </div>
      )}
